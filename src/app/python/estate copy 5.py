...
try:
    ...
    while True:
        ...
        # 図面取得
        bukken_nums = driver.find_element(By.CSS_SELECTOR, 'div.tab-content > div > div > div.p-table.small > div.p-table-body')
        bukken_num = bukken_nums.driver.find_element_by_css_selector("div.tab-content > div > div > div.p-table.small > div.p-table-body")
        for b in bukken_nums:
            bukken_num = b.driver.find_element_by_css_selector("div:nth-child(" + str(b + 1) + ") > div:nth-child(4)").text
            detail_elems = b.driver.find_elements(by=By.XPATH, value="//button[contains(@class, 'btn p-button m-0 py-0 btn-outline btn-block px-0') and contains(., '図面')]")
            print(bukken_num)
    #__BVID__3435 > div > div.p-table.small > div.p-table-body > div:nth-child(1) > div:nth-child(27) > button
# //*[@id="__BVID__3435"]/div/div[2]/div[2]/div[1]/div[27]/button
        for i in range(detail_count):

            # /html/body/div/div/div/div[1]/div[1]/div/div[2]/div/div[2]/div/div/div[2]/div[2]/div[1]/div[4]
            # driver.find_elements
            
            # #div.tab-content > div > div > div.p-table.small > div.p-table-body > div:nth-child(1) > div:nth-child(4)
            # #div.tab-content > div > div > div.p-table.small > div.p-table-body > div:nth-child(2) > div:nth-child(4)
            # # 物件図面 
            # # 物件図面の有無を判断
            # if len(driver.find_elements(by=By.XPATH, value="//*[@id='__layout']/div/div[1]/div[1]/div/div[21]/div/div/div/div[2]/div[1]")) > 0 :
            #     # driver.save_screenshot(SSDIR)
            #     print('図面PDFファイル保存開始：' + property_num.text)
            #     # time.sleep(13)
            ...
            #             driver.save_screenshot(os.path.join(SSDIR, "screen" + str(j) + "-1.png"))
            ...
            #                     # == エラー処理をここに記載 ==
            #                     # 終了処理
            #                     print(property_num.text + 'の図面pdf取得に失敗しました。')
            #                     driver.refresh()
            #                     time.sleep(4)
            #                     driver.save_screenshot(os.path.join(SSDIR, "screen" + str(j) + "-2.png"))
