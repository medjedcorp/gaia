            ...
            if len(driver.find_elements(by=By.XPATH, value="//*[@id='__layout']/div/div[1]/div[1]/div/div[21]/div/div/div/div[2]/div[1]")) > 0 :
                ...
                for j in range(5):
                    if pdf_flag:
                        ...
                        time.sleep(2)
                        # driver.find_element(By.CLASS_NAME, ".btn .p-button .btn-outline").click()
                        # driver.find_element(by=By.css_selector, value=".btn .p-button .btn-outline").click()
                        # driver.execute_script('arguments[0].click();', zumen)
                        # zumen = driver.find_element(By.LINK_TEXT, "図面参照")
                        # zumen_btn = webdriver.ActionChains(driver).move_to_element(zumen).perform()
                        zumen.click()
                        print(zumen.text)
                        # driver.execute_script('document.querySelector("#__layout > div > div.p-frame-main > div.p-frame-content > div > div:nth-child(48) > div > div > div > div.row > div:nth-child(2) > button").click();')
                        
                        # if j % 2 == 0:
                        #     # driver.execute_script('document.querySelector("#__layout > div > div.p-frame-main > div.p-frame-content > div > div:nth-child(48) > div > div > div > div.row > div:nth-child(2) > button").click();')
                        #     # print('querySelectorでボタンをクリックします')
                        #     #__layout > div > div.p-frame-footer > div > div > div > div > div:nth-child(3) > button
                        #     zumen = driver.find_element(by=By.XPATH, value="//*[@id='__layout']/div/div[1]/div[1]/div/div[21]/div/div/div/div[2]/div[2]/button")
                        #     print('XPATH1でボタンをクリックします')
                        #     zumen.click()
                        # else:
                        #     print('XPATH2でボタンをクリックします')
                        #     zumen.click()
