		...
		<div class="card overflow-scroll mt-2" style="height: 70vh">
			<div class="card-body">
				...
				<div class="d-flex align-items-center text-dark">
					...
					@else
					<img src="images/noimage.png" data-src="images/{{$land->bukken_num}}/{{$land->photo1}}" class="align-self-start p-1 border lazyload" width="90" height="90" alt="{{$land->bukken_num}}">
					@endif
...
var markerData = [ // マーカーを立てる場所名・緯度・経度
    @foreach($lands as $land)
	{
        ...
        land_menseki: `{{ $land->youseki_rate }}`,
    ...
    function success(pos) {
		...
		for (var i = 0; i < markerData.length; i++) {
			...
			infoWindow[i] = new google.maps.InfoWindow({ // 吹き出しの追加
         		content: '<a href="/landshow/'+ markerData[i]['bukken_num'] +'"><div class="row g-0 text-dark"><div class="col-md-4"><img src="images/'+ markerData[i]['photo1']  +'" alt="' + markerData[i]['bukken_num'] + '" style="max-width:100px; max-height:100px;"></div><div class="col-md-8 ps-1"><div class="card-body"><h6 class="card-title"><div class="badge bg-primary" style="display: block; margin-bottom:5px;">' + markerData[i]['bukken_shumoku'] + '</div><span>' + markerData[i]['address1'] + markerData[i]['address2'] + markerData[i]['address3'] + '</span></h6><div class="clearfix"><h6 class="mb-0 float-start text-danger"><span class=" fs-5">' + markerData[i]['price'] + '</span><span class="fs-6">万円</span></h6></div><p class="card-text">土地面積：' + markerData[i]['land_menseki'] + '&#13217;<br>建ぺい率：' + markerData[i]['kenpei_rate'] + '<br>容積率：' + markerData[i]['land_menseki'] + '％<br>建築条件：' + markerData[i]['kenchiku_jyouken'] + '</p></div></div></div><div class="row g-0"><div class="col-12"><div type="button" class="btn btn-success px-5 d-block"><i class="fadeIn animated bx bx-detail"></i>詳細を表示</div></div></div></a>' // 吹き出しに表示する内容
       		});
